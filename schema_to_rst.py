...
env = Environment(
    loader=FileSystemLoader(os.path.join(os.path.dirname(__file__), '_templates', 'schema')),
    trim_blocks=True)
...
AVRO_PRIMITIVE_TYPES = {'null', 'int', 'long', 'float', 'double', 'bytes', 'string', 'boolean'}
...
def dictify_type(typ):
    if isinstance(typ, str):
        return {'type': typ, 'nullable': False, 'is_avro_primitive': is_avro_primitive(typ)}
    elif isinstance(typ, dict):
        return {**typ, 'nullable': False, 'is_avro_primitive': is_avro_primitive(typ['type'])}
    elif isinstance(typ, list) and 'null' in typ:
        non_null = list(set(typ) - set(['null']))[0]
        return {
            'type': non_null,
            'nullable': True,
            'is_avro_primitive': is_avro_primitive(non_null)
        }
    else:
        raise RuntimeError("Unsupported type: {0}".format(typ))


LINK=re.compile('(?P<namespace>[a-z_\.]+)*\.(?P<type>[a-z0-9_]+)(#(?P<field>[a-z0-9_]+))*', re.IGNORECASE)
        ...
        for typ in protocol['types']:
            for field in typ.get('fields', []):
                ...
                if dct.get('origin') == origin:
                    yield '{0}.{1}#{2}'.format(protocol['namespace'], typ['name'], field['name'])
    ...
    for protocol_namespace, protocol in protocols_map.items():
        for record in protocol.get('types'):
            ...
            yield {
                ...
                'bq_table': bq_table,
                'doc': record.get('doc'),
                'fields' : [ build_field(record.get('namespace', protocol_namespace), record.get('name'), f) for f in record.get('fields')]
            }


def build_docs2(protocol_map):
...
def parse_arguments():
    parser = argparse.ArgumentParser(description='Generates schema docs from avro protocol files')
...
def main():
    ...
    print(glob.glob(os.path.join(args.protocol_path, '*.avpr')))
    protocols = [ json.load(open(filename, 'r')) for filename in glob.glob(os.path.join(args.protocol_path, '*.avpr'))]

    protocols_map = { p['namespace']: p for p in protocols}
    for file_name, content in build_docs2(protocols_map):
